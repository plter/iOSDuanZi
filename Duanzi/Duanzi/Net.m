...
+(void)loadData:(NSString *)url args:(NSDictionary *)dict completeHandler:(void (^)(NSData *))handler{
    ...
    [NSURLConnection sendAsynchronousRequest:req queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        ...
        if (data) {
            ...
            [[[UIAlertView alloc] initWithTitle:@"提醒" message:@"加载数据失败，请稍候重试" delegate:nil cancelButtonTitle:@"好的" otherButtonTitles:nil, nil] show];
        }
    }];
}


+(void)regWithIDFA:(NSString *)idfa completeHandler:(void (^)(NSDictionary *))handler{
...
+(void)getUid:(void (^)(NSString *))callback{
    ...
    if (uid) {
        ...
        [[App getInstance] showLoading:@"正在通信，请稍候"];
    ...
    [Net loadData:GET_LIST_URL args:dict completeHandler:^(NSData *data) {
        ...
        if (error) {
            ...
            if ([[result valueForKey:@"rs"] isEqualToString:@"1"]) {
                ...
                [[[UIAlertView alloc] initWithTitle:@"提醒" message:@"加载数据失败，请稍候重试" delegate:nil cancelButtonTitle:@"关闭" otherButtonTitles:nil, nil] show];
            }
        }
    }];
...
+(void)userAction:(NSString *)uid action:(USER_ACTION)action jid:(NSString *)jid completeHandler:(void (^)(BOOL))handler{
    ...
    [[App getInstance] showLoading:@"正在通信，请稍候"];
    [Net loadData:USER_ACTION_URL args:[[NSDictionary alloc] initWithObjectsAndKeys:uid,@"uid",uploadAction,@"action",jid,@"aid", nil] completeHandler:^(NSData *data) {
        ...
        if (err) {
            [[[UIAlertView alloc] initWithTitle:@"提醒" message:@"服务端返回的数据无效" delegate:nil cancelButtonTitle:@"关闭" otherButtonTitles:nil, nil] show];
        }else{
